export default {
    ...
    start: async (handler, args) => {
        ...
        let PAGE_LIST = JSON.parse(handler.fs.get('/tmp/blog_pagelist').content);
        let FRIEND_LIST = JSON.parse(handler.fs.get('/tmp/blog_friendlist').content);
        let moo_count = JSON.parse(handler.fs.get('/tmp/blog_moocount').content);
        if (PAGE_LIST.length == 0 || FRIEND_LIST.length == 0) {
            const temp = handler.term.cursor, jobs = (PAGE_LIST == null ? 1 : 0) + (FRIEND_LIST == null ? 1 : 0);
            if (PAGE_LIST.length == 0) {
                ...
                try {
                    ...
                    if (t.ok) {
                        ...
                        throw new Error(`/blog/page.json request failed(${t.status} ${t.statusText})`);
                    }
                }
                catch (err) {
                    console.error(err);
            ...
            if (FRIEND_LIST.length == 0) {
                ...
                try {
                    ...
                    if (t.ok) {
                        ...
                        throw new Error(`/blog/friend.json request failed(${t.status} ${t.statusText})`);
                    }
                }
                catch (err) {
                    console.error(err);
        ...
        switch (args[0] ?? 'help') {
            ...
            case 'moo': {
                switch (moo_count++) {
                    ...
                    case 2: {
                        handler.term.write('你是想要兽人什么的吗？凌就是兽人来着，但请你不要在这里调戏他的博客。\n');
                        break;
                    }
                    case 3: {
                    ...
                    case 4: {
                        handler.term.write('你就是想涩涩对吧？我满足你的话，能别来烦我了吗？\n');
                        break;
                    }
                    case 5: {
                        handler.term.write('我佩服你的毅力。\n诺，这是你要的涩涩链接:', Link('涩涩！', 'https://5e.fit/ZMInO'), '\n');
                        break;
                    }
                    case 6: {
                ...
                if (args.length == 2) {
                    ...
                    for (const i in PAGE_LIST) {
                        if (PAGE_LIST[i].title.indexOf(index) != -1) {
                            sum++;
                            handler.term.write(`${i} "${PAGE_LIST[i].title}" ${PAGE_LIST[i].author} 作于 ${new Date(PAGE_LIST[i].date).toUTCString()}\n`);
                        }
                    }
            ...
            case 'about': {
                try {
                    ...
                    handler.term.write(`读入页面异常(${val.status} ${val.statusText})。\n`);
                }
                catch (err) {
                    console.error(err);
            ...
            case 'friend': {
                ...
                for (const i in FRIEND_LIST) {
                    handler.term.write(`${i} `, Link(FRIEND_LIST[i].name, FRIEND_LIST[i].url), ` ${FRIEND_LIST[i].desc}\n`);
                }
